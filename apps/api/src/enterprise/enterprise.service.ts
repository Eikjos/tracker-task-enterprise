...
import { EtablissementResponse, SireneAuthentification } from "types/sirene-api";
...
export class EnterpriseService {
  ...
  async getInformationBySiret(siret: string) : Promise<EnterpriseInformationDto> {
    ...
    return {
      ...
      address: insee.etablissement.adresseEtablissement.numeroVoieEtablissement
      ...
      city: insee.etablissement.adresseEtablissement.libelleCommuneEtablissement,
      zipCode: insee.etablissement.adresseEtablissement.codePostalEtablissement,
      legalStatus: insee.etablissement.uniteLegale.categorieJuridiqueUniteLegale,
      socialCapital: insee.etablissement.uniteLegale.capitalSocialUniteLegale,
      numberTva: "FR" + this.getCleControleTVANumber(insee.etablissement.siren) + insee.etablissement.siren,
    } 
  }
  ...
  // -- Tools --
   private async getInseeInformation(siret : string) {
    ...
    const token = await this.httpService.axiosRef.post<SireneAuthentification>("https://api.insee.fr/token", "grant_type=client_credentials", { headers : header});
    // Récupération des informations de l'entreprise
    const url = "https://api.insee.fr/entreprises/sirene/V3.11/siret/" + siret;
    const response = await this.httpService.axiosRef.get<EtablissementResponse>(url, { headers : { Authorization: 'Bearer ' + token.data.access_token}});
