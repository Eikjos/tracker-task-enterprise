...
<template>
  <div class="flex flex-col gap-1" :class="props.class">
    ...
    <input
      ...
      :class="
        cn(
          `flex h-10 w-full rounded-xs border-b ${props.error ? 'border-red-600' : 'border-input'}
       bg-transparent px-2 py-1 text-sm file:border-0 file:bg-transparent file:text-sm file:font-medium
       placeholder:text-muted-foreground focus:border-primary focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50`
        )
      "
      @focusin="focusIn"
      @focusout="focusOut"
