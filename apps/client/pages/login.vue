<template>
  <div class="flex">
    <div class="w-2/3 flex flex-col justify-center">
      ...
      <span class="text-center text-2xl font-bold w-1/2 mx-auto mt-20"
        >Gérer votre vie d'entrepreneur libre facilement</span
      >
    </div>
    <form
      class="w-1/3 border-l-2 border-inpu dark:border-slate-700 px-10 flex flex-col gap-2 h-screen bg-white dark:bg-slate-900"
      @submit="onSubmit"
    >
      <img
        src="/img/png/logo.png"
        class="w-[200px] h-[50px] mx-auto mb-20 mt-10"
      />
      ...
      <span class="text-xs"
        >Mot de passe oublié ?
        <a class="cursor-pointer text-gray-700 dark:text-slate-300"
          >Modifiez-le</a
        ></span
      >
      <Button type="submit" class="mt-4">Se connecter</Button>
      <span v-if="error" class="text-center text-red-700 text-xs">{{
        error
      }}</span>
    </form>
  </div>
...
import type { LoginData } from "@repo/models";
