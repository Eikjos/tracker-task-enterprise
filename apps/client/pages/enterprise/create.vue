<template>
  ...
  <form v-if="step === 1" class="w-1/2 mx-auto mt-5 bg-accent rounded-lg border-2 border-input p-5">
      ...
      <Input label="Statut légale" v-model="legalStatus" type="text" name="siret" placeholder="Numéro de siret" :error="form2.errors.value.legalStatus"/>
...
const createEnterpriseSchema = toTypedSchema(
  z.object({
    ...
    legalStatus: z.string({required_error: "Le statut de l'entreprise est requis."}),
...
const [legalStatus] = form2.defineField('legalStatus');
